...
void CRGBA::RainbowCycle(uint32 t) {
    ...
    // Âû÷èñëÿåì ïðîøåäøåå âðåìÿ ñ íà÷àëà ïåðèîäà
    uint32_t timeElapsed = t % period;

    // Âû÷èñëÿåì ïðîãðåññ â ïåðèîäå îò 0 äî 1
    double progress = static_cast<double>(timeElapsed) / static_cast<double>(period);

    // Èñïîëüçóåì sin äëÿ ïëàâíîãî ïåðåõîäà ìåæäó öâåòàìè
    double angle = progress * 2 * M_PI; // Ïðåîáðàçóåì ïðîãðåññ â óãîë (0 - 2?)
    r = sin(angle + 0) * 127 + 128; // Ãåíåðèðóåì êîìïîíåíòû öâåòà
